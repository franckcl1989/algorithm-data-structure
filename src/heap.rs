...
// 4. 利用堆求中位数
// 中位数即中间位置得数，如果数列为奇数将数据从小到大排列中位数为 n / 2 + 1（数据下标从0开始）
// 如果数列为偶数则中位数有2个，分别是 n / 2 和 n / 2 + 1 可以任取一位作为中位数
// 对于静态数据可以直接进行排序然后取中位数虽然排序代价较大但是边际成本很小，如果是动态数据则排序取中位数不太适用
// 对于动态数据取中位数可以通过堆这种数据结构实现：
// 维护两个堆 一个大顶堆 一个小顶堆 大顶堆存储数据的前半部分 小顶堆存储数据的后半部分 且小顶堆中的数据都大于大顶堆中的数据
// 将原始数列从小到大拆分，如果为奇数 前半部分为 n / 2 + 1 后半部分为 n /2 如果为偶数 前半部分为 n / 2 后半部分为 n / 2
// 前半部分存入大顶堆，后半部分存入小顶堆，当新加入得数据小于大顶堆得堆顶元素则将其插入大顶堆 否则插入小顶堆
// 因为频繁插入数据可能造成两个堆大小不平衡得情况（不符合前后部分得长度），这时可以从一个堆得堆顶元素移动到另一个堆
// 插入数据涉及堆化时间复杂度为 O(logn) 获取中位数可以直接取大顶堆得堆顶元素 时间复杂度为 O(1)
// 5. 利用堆求百分位数据
// 在性能、监控领域经常会有这样得衡量指标 99%、95%、90% line 这类百分比数据也可以通过两个堆来实现原理类似求中位数
// 维护两个堆 一个大顶堆 一个小顶堆 大顶堆中保存 n% 的数据 小顶堆保存 （100-n）% 得数据
// 新数据插入时判断与两个堆顶元素得大小，小于大顶堆堆顶则插入大顶堆，大于小顶堆堆顶则插入小顶堆
// 根据 n% 比例 移动堆中得元素到另一个堆，这样求 n% line 时直接返回大顶堆得数据即可 时间复杂度是 O(1)